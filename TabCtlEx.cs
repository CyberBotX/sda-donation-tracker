...
namespace SDA_DonationTracker
{
	...
	/// </summary>
	public class TabCtlEx : TabControl
	{
		/// <summary>
					...
					if (nIndex != this.SelectedIndex)
					{
						...
						using (GraphicsPath _Path = new GraphicsPath())
						{
							...
							using (LinearGradientBrush _Brush = new LinearGradientBrush(tabTextArea, SystemColors.Control, SystemColors.ControlLight, LinearGradientMode.Vertical))
							{
								...
								if (this.CanDrawMenuButton(nIndex))
								{
									_ColorBlend.Positions = new float[] { 0f, .4f, 0.5f, 1f };
		...
		{
			if ((this.TabPages[nIndex] as TabPageEx).Menu != null)
				return true;
			return false;
		}
			...
			if (!this.DesignMode)
			{
				using (Graphics g = CreateGraphics())
				{
					...
					for (int nIndex = 0; nIndex < this.TabCount; ++nIndex)
					{
						...
						tabTextArea = new RectangleF(tabTextArea.X + tabTextArea.Width - 22, 4,
							tabTextArea.Height - 3, tabTextArea.Height - 5);
						...
						if (this.CanDrawMenuButton(nIndex))
						{
							...
							tabMenuArea = new RectangleF(tabMenuArea.X + tabMenuArea.Width - 43, 4,
								tabMenuArea.Height - 3, tabMenuArea.Height - 5);
				...
				if (tabTextArea.Contains(pt))
				{
					if (this.ConfirmOnClose)
					{
						if (MessageBox.Show("You are about to close " +
								this.TabPages[this.SelectedIndex].Text.TrimEnd() +
					...
					if (this.OnClose != null)
						this.OnClose(this, new CloseEventArgs(this.SelectedIndex));
					return;
				}
				if (this.CanDrawMenuButton(this.SelectedIndex))
				{
					RectangleF tabMenuArea = (RectangleF)this.GetTabRect(this.SelectedIndex);
					tabMenuArea = new RectangleF(tabMenuArea.X + tabMenuArea.Width - 43, 4,
						tabMenuArea.Height - 3, tabMenuArea.Height - 5);
					pt = new Point(e.X, e.Y);
					if (tabMenuArea.Contains(pt) && (this.TabPages[SelectedIndex] as TabPageEx).Menu != null)
						(this.TabPages[SelectedIndex] as TabPageEx).Menu.Show(this,
							new Point((int)tabMenuArea.X, (int)(tabMenuArea.Y + tabMenuArea.Height)));
				}
			}
		}
	}
